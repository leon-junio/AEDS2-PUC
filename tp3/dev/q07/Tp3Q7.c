...
void gerarLog(double tempo)
{
    ...
    file = fopen("1369371_countingsort.txt", "w");
    fputs("1369371", file);
    fputs("\t", file);
    fputs(cp, file);
    fputs("\t", file);
...
int getMaior()
{
    int maior = filmes[0].duracao;
    for (int i = 0; i < MAXTAM; i++)
    {
        comp++;
        if (maior < filmes[i].duracao)
        {
...
// Ordenação por COUNTINGSORT em C
void ordenar()
{
    int len = getMaior() + 1;
    mov++;
    int count[len];
    Filme ordenado[MAXTAM];
    for (int i = 0; i < len; count[i] = 0, i++)
        ;
    mov += len;
    for (int i = 0; i < MAXTAM; count[filmes[i].duracao]++, i++)
        ;
    for (int i = 1; i < len; count[i] += count[i - 1], i++)
        ;
    for (int i = MAXTAM - 1; i >= 0; ordenado[count[filmes[i].duracao] - 1] = filmes[i], count[filmes[i].duracao]--, i--)
        ;
    mov += MAXTAM;
    for (int i = 0; i < MAXTAM; filmes[i] = ordenado[i], i++)
        ;
    mov += MAXTAM;
}

/**
 * codigo de ordenação por inserção em C com Filmes
 */
void insertionSort()
{
    int mov = 0, comp = 0;
    for (int i = 1; i < MAXTAM; i++)
    {
        ...
        mov += 2;
        comp++;
        while ((j >= 0) && filmes[j].duracao > tmp.duracao)
        {
            filmes[j + 1] = filmes[j];
            j--;
            mov++;
