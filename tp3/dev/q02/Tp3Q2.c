...
void gerarLog(double tempo)
{
	...
	file = fopen("1369371_selecaoRecursiva.txt", "w");
	fputs("1369371", file);
	fputs("\t", file);
	fputs(cp, file);
	fputs("\t", file);
...
// Ordenação por seleção recursiva em C
void ordenar(int menor, int maior)
{
	...
	if (menor < maior)
	{
		...
		{
			comp++;
			if (isStrMaior(filmes[j].titulo, filmes[i].titulo))
			{
				j = i;
				mov++;
			}
		...
		temp = filmes[menor];
		filmes[menor] = filmes[j];
		filmes[j] = temp;
		mov += 3;
		ordenar(menor + 1, maior);
	}
}
