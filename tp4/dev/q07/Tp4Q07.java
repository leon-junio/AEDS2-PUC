...
class Filme {
	...
	private void ler(String entrada) {
		try {
			...
			line = buff.readLine();
			boolean chk = false;
			while (!Ferramentas.myContains(line, "TÌtulo original")) {
				line = buff.readLine();
				if (Ferramentas.myContains(line, "<bdi>SituaÁ„o</bdi>")) {
					setTitulo(getNome());
					chk = true;
					break;
				}
			...
			while (!Ferramentas.myContains(line, "SituaÁ„o")) {
				line = buff.readLine();
			}
			...
			line = buff.readLine();
			while (!Ferramentas.myContains(line, "OrÁamento")) {
				line = buff.readLine();
			}
	...
	private String tratarLinha(String linha, int op) {
		...
		switch (op) {
			...
			case 3:
				...
				break;
			case 4:
				resp = Ferramentas.myReplace(linha, "<p class=\"wrap\"><strong>TÌtulo original</strong> ", "");
				resp = Ferramentas.myReplace(resp, "</p>", "");
				break;
			case 5:
				resp = Ferramentas.myReplace(linha, "<strong><bdi>SituaÁ„o</bdi></strong> ", "");
				break;
			case 6:
				...
				break;
			case 7:
				resp = Ferramentas.myReplace(linha, "<p><strong><bdi>OrÁamento</bdi></strong> ", "");
				resp = Ferramentas.myReplace(resp, "</p>", "");
...
class Lista {
	...
	public void inserir(Filme fm, int pos) throws Exception {
		...
		if (pos == 0) {
			...
			throw new Exception("Posi√ß√£o de inser√ß√£o invalida - pos:" + pos + " size:" + tam);
		} else {
			Celula i = primeiro;
	...
	public Filme remover(int pos) throws Exception {
		...
		if (primeiro == ultimo) {
			...
			throw new Exception("Posi√ß√£o de remo√ß√£o inv√°lida nesse contexto! pos:" + pos + " size:" + tamanho);
		} else if (pos == 0) {
			removerInicio();
	...
	public Filme localizar(int pos) throws Exception {
		...
		if (primeiro == ultimo) {
			...
			throw new Exception("Posi√ß√£o para busca inv√°lida na lista!");
		} else {
			Celula i = primeiro;
