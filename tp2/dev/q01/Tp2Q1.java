...
class Filme {
	...
	//Função de clonagem de objeto para evitar erros de acesso de memoria e perda de dados
	public Filme clonar() {
	...
	//Método que vai iniciar o objeto de Filme e realizar a pesquisa entre as linhas
	//do arquivo atrás dos dados necessários para a extração. Esse método realiza vários
	...
	//Função que retorna um arquivo de html para poder ser realizado as consultas e extração de dados
...
class Lista {
	...
	/*private boolean findSequencial(String nome){
		boolean resp = false;
		for (Filme filmeObj : filmes) {
			if(filmeObj.getNome().equals(nome)){
				resp = true;
			}
		}
		return resp;
